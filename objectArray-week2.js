...
//fiyati 30 ile 100 arasinda kac adet urun var?
//ismi a harfi ile baslayan urunleri console a yazdir
//ismi a harfi ile BITEN urunleri console a yazdir
// id si 10 olan urunun adini console a yazdir
//discontinued false olan kac urun var
// en pahali urunun adini console a yazdir
//en ucuz urunun adini console a yazdir

//fiyati 30-100 arasinda olan
let priceCount = 0;
//ismi a harfi ile basalayan array
let nameStartsA = [];
//ismi a harfi ile biten array
let nameEndsA = [];
//id=10
let product;
//discontinued=false sayaci
let discontinuedFalseCounter = 0;
//en pahali urun
let priceMax = products[0].unitPrice;
let priceMaxName;
//en ucuz urun
let priceMin = products[0].unitPrice;
let priceMinName;

for (let i = 0; i < products.length; i++) {
  if (products[i].unitPrice > 30 && products[i].unitPrice < 100) {
    priceCount++;
  }
  if (products[i].name.toLowerCase().charAt(0) === "a") {
    nameStartsA.push(products[i].name);
  }
  if (products[i].name.toLowerCase().charAt(products[i].name.length - 1) === "a") {
    nameEndsA.push(products[i].name);
  }
  if (products[i].id === 10) {
    product = products[i].name;
  }
  if (products[i].discontinued === false) {
    discontinuedFalseCounter++;
  }
  if (priceMax < products[i].unitPrice) {
    priceMax = products[i].unitPrice;
    priceMaxName = products[i].name;
  }
  if (priceMin > products[i].unitPrice) {
    priceMin = products[i].unitPrice;
    priceMinName = products[i].name;
  }
}

console.log("fiyati 30 ile 100 arasinda olan urun sayisi:", priceCount);
console.log("ismi a ile baslayan urunler:", nameStartsA.join(", "));
console.log("ismi a ile biten urunler:", nameEndsA.join(", "));
